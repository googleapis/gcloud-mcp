...
export const lint = async (command: string): Promise<ParsedGcloudLintResult> => {
  const { code, stdout, stderr } = await invoke(['meta', 'lint-gcloud-commands', '--command-string', `gcloud ${command}`]);
  ...
  return {
    success: true,
    parsedCommand: lintCommand.command_string_no_args.slice("gcloud ".length) // Remove gcloud prefix
  };
}
